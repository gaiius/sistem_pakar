...
public class ListGejalaAdapter extends ArrayAdapter {
	private final Activity activity;
    private final List<JSONObject> list;

    @SuppressWarnings("unchecked")
	public ListGejalaAdapter(Activity activity, int view, List<JSONObject> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
    	...
    	if(gjl != null){
    		...
    		if(subText != null){
    			try {
					subText.setText(gjl.getString("nm_handphone"));
				} catch (JSONException e) {
					subText.setText("key nm_handphone NOT FOUND");
				}
    		}
    	}
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if(constraint != null && constraint.toString().length() > 0)
            {
                ...
                for(int i = 0, l = allModelItemsArray.size(); i < l; i++)
                {
                	...
                	try{
	                    if(m.getString("nm_gejala").toLowerCase().contains(constraint))
	                        filteredItems.add(m);
                	} catch(Exception e){
